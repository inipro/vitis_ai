...
static cv::Mat process_result(cv::Mat &image,
  ...
  for (const auto bbox : result.bboxes) {
    ...
    class_label = classes[bbox.label];
    ...
    LOG_IF(INFO, is_jpeg) << "RESULT: " << label << "\t" << xmin << "\t" << ymin
                          ...
                          << "\n";
      cv::rectangle(image, cv::Point(xmin, ymin), cv::Point(xmax, ymax),  getColor(label), 2, 1, 0);
      cv::putText(image, class_label, cvPoint(xmin, ymin), cv::FONT_HERSHEY_SIMPLEX, 1.0, getColor(label), 1.0);
