...
void CentroidTracker::register_Object(int cX, int cY, std::vector<int> bbox) {
    int object_ID = this->nextObjectID;
    this->objects.push_back({object_ID, {cX, cY}});
    this->bboxes.push_back({object_ID, bbox});
...
std::vector<std::pair<int, std::vector<int>>> CentroidTracker::update(const vitis::ai::FaceDetectResult &result) {
    if (result.rects.empty()) {
        ...
        while (it != this->disappeared.end()) {
            ...
            if (it->second > this->maxDisappeared) {
                ...
                this->bboxes.erase(remove_if(this->bboxes.begin(), this->bboxes.end(), [it](auto &elem) {
                    return elem.first == it->first;
                }), this->bboxes.end());
    ...
    vector<vector<int>> inputBBoxes;
    for (const auto &r : result.rects) {
        int cX = int((r.x + r.width/2.0)*result.width);
        int cY = int((r.y + r.height/2.0)*result.height);
        inputCentroids.push_back(make_pair(cX, cY));
        inputBBoxes.push_back({int(r.x*result.width), int(r.y*result.height), int(r.width*result.width), int(r.height*result.height)});
    }
    ...
    if (this->objects.empty()) {
        for (int i=0; i<inputCentroids.size(); i++) {
            this->register_Object(inputCentroids[i].first, inputCentroids[i].second, inputBBoxes[i]);
        }
    }
        ...
        for (int i = 0; i < rows.size(); i++) {
            ...
            for (int t = 0; t < this->objects.size(); t++) {
                if (this->objects[t].first == objectID) {
                    ...
                    this->bboxes[t].second = inputBBoxes[cols[i]];
                }
            }
        ...
        if (objectCentroids.size() >= inputCentroids.size()) {
            ...
            for (auto row: unusedRows) {
                ...
                if (this->disappeared[objectID] > this->maxDisappeared) {
                    ...
                    this->bboxes.erase(remove_if(this->bboxes.begin(), this->bboxes.end(), [objectID](auto &elem) {
                        return elem.first == objectID;
                    }), this->bboxes.end());
            ...
            for (auto col: unusedCols) {
                this->register_Object(inputCentroids[col].first, inputCentroids[col].second, inputBBoxes[col]);
            }
        }
    }
